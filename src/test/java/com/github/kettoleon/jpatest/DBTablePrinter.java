            ...
            while (rs.next()) {
                ...
                for (int i = 0; i < columnCount; i++) {
                    ...
                    if (category == CATEGORY_OTHER) {
                        ...
                        if("VARBINARY".equals(c.getTypeName())){
                            value = HexBin.encode(rs.getBytes(i+1));
                            if(value == null){
                                value = "";
                            }
                        }else if("CLOB".equals(c.getTypeName())){

                            try {
                                value = IOUtils.toString(rs.getClob(i + 1).getCharacterStream());
                            } catch (IOException e) {
